...
import { instanceTodo, TodoT } from "../TodoStorage/Todo.js"
...
const eventActionListTodolist = event(
...
export const pushTodolist = (todo: TodoT) => {
    let listTodolistContent = ""
    todo.listTodo.map(({ id, time, name }) => {
        if (id) listTodolistContent += listTodolist(time, name, id, todo.id)
    })

    el(".todolist-content").innerHTML += todolist(
        todo.id,
        todo.title,
        listTodolistContent
    )
...
const todolist = (id: number, title: string, listTodolist: string): string => {
    return /* html */ `
